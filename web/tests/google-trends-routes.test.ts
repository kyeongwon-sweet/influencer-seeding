...
test("Google search trends collector and webhook bypass Clerk but keep their own secrets", () => {
  ...
  assert.match(collectRoute, /"signalbench\/google-trends-scraper"/);
  assert.match(collectRoute, /searchTerms: keywords/);
  assert.match(collectRoute, /timeRange: "today 3-m"/);
  assert.match(collectRoute, /geo: "KR"/);
  assert.match(collectRoute, /includeInterestOverTime: true/);
  assert.match(collectRoute, /apifyProxyGroups: \["RESIDENTIAL"\]/);
  ...
  assert.match(webhookRoute, /parseGoogleTrendDataset\(items\)/);
