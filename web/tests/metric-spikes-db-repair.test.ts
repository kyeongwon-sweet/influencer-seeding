...
test("9/3 DB repair targets only the five confirmed banner reach rows", () => {
  ...
  assert.match(route, /update\(\{ reach_count: null \}\)/);
  assert.doesNotMatch(route, /update\(\{ play_count: null \}\)/);
