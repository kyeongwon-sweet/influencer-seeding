...
const PLATFORM_LABEL: Record<string, string> = {
  ...
  tiktok: "틱톡", 틱톡: "틱톡",
  // 2026-08-06 사용자 지시: 표기를 '트위터'로 통일(이전엔 'X'로 보였다).
  // 저장값은 영문 슬러그(`x`·`twitter`)와 한글이 섞여 있는데(자동수집=영문, 수동=한글)
  // 표시는 이 한 곳에서만 결정되므로 DB를 건드리지 않고 라벨만 바꾼다.
  twitter: "트위터", x: "트위터", 트위터: "트위터", 엑스: "트위터",
  threads: "스레드", 스레드: "스레드",
