...
export default function MonitoringPage() {
  ...
  async function loadPosts() {
    ...
    const decodedPosts: Post[] = (Array.isArray(json) ? json : []).map((p: Post & { stats_v2?: unknown }) => (
      p.stats_v2 !== undefined ? { ...p, all_stats: decodeStatsV2(p.stats_v2) } : p
    ));
    const { rows: uniquePosts, duplicateIds } = dedupeRowsById(decodedPosts);
    if (duplicateIds.length > 0) {
      console.error(`[monitoring] API duplicate post ids removed (${duplicateIds.length})`, duplicateIds.slice(0, 10));
    }
    let newPosts = uniquePosts;
