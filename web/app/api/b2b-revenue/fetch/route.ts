...
export async function GET(req: NextRequest) {
  ...
  // TEMP: 연동 시트 자취생(DYFBwz5GlJ7) 행 날짜별 값 조회(시트 정리 지침용). 확인 후 제거.
  if (req.nextUrl.searchParams.get("jachwi")) {
    ...
    const out: { row: number; account: string; values: Record<string, string | number> }[] = [];
    for (let i = 1; i < rows.length; i++) {
      ...
      if (String(r[cUrl] ?? "").includes("DYFBwz5GlJ7")) {
        ...
        out.push({ row: i + 1, account: String(r[cAcc] ?? "").trim(), values });
      }
    }
