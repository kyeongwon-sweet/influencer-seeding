...
export async function GET(req: NextRequest) {
  ...
  // TEMP: 연동 시트에서 썰박스/썰뜨기 채널의 업체명(D열) 채워진 셀 찾기. 확인 후 제거.
  if (req.nextUrl.searchParams.get("sulbox")) {
    ...
    const colLetter = String.fromCharCode(65 + cCo); // 업체명 열 문자
    return NextResponse.json({ companyCol: colLetter, cAcc, cCo, count: hits.length, hits });
  }
