...
### 검증 우선 원칙
- 대시보드/코드베이스 작업에서 추측으로 답하지 않는다.
- 사용자에게 사실을 말하기 전에는 코드, 데이터, 또는 실제 실행 결과로 먼저 검증한다.
- 코드 변경 전에는 그 결정이 번복 없이 확실한지 다시 점검한다.
- 변경 완료 후에는 코드만 보지 말고 실제 반영 여부를 검증한다. 가능하면 UI/런타임 확인을 우선하고, 테스트/빌드는 보조 근거로 함께 사용한다.
- 실제 검증이 막히면 무엇을 검증하지 못했는지와 이유를 명확히 말한다.
