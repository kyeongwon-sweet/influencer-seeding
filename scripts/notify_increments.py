...
def main():
    ...
    # 저장 increment는 오염될 수 있으므로 읽지 않고, 대시보드 safeIncrement와 같은 규칙으로 재계산한다.
    # 증분 = target일 측정값 - 이전 유효(>0) 누적 최댓값. 이전 유효값이 없으면 첫 측정값 전체.
    ...
    def _safe_inc(rows, isb):
        ...
        if cur is None or cur <= 0:
            return None
        return cur if not has else max(0, cur - base)

    # items: 저장 increment 대신 안전 규칙으로 재계산한다.
    items = []
    for pid in jd_pids:
        m = meta.get(pid, {})
        ...
        cum = max([(_metric(r, isb) or 0) for r in rows] or [0])
        items.append({
            "inc": inc,
