...
def main():
    ...
    #    ⚠️ 08-31로 계획 종료 — 08-27~ 300만 → 09-06~ 190만(사용자 지시 2026-09-14).
    _GOAL_TIERS = [
        ("2026-09-06", 1_900_000),  # 9/6~: 목표 190만(사용자 지시 2026-09-14). 지난주(대상 9/6~)부터 적용, 이후 유지.
        ("2026-08-27", 3_000_000),  # 8/27~9/5: 목표 300만(사용자 지시 2026-08-27).
    ...
    # REPORT_NOTE — 그날 숫자를 곧이곧대로 읽으면 오해가 생기는 경우의 주석 한 줄(사용자 승인 시에만).
    #   예: 수집기 장애 복구로 값이 소급 반영돼 증분에 하루치 이상이 섞인 날.
    #   ⚠️ 값을 바꾸지 않는다 — 표시된 숫자가 무엇인지 설명만 한다. 미설정이면 아무것도 안 붙는다.
    _note = (os.getenv("REPORT_NOTE") or "").strip()
    if _note:
        lines.append(f"_{_esc(_note)}_")
    lines += ["", DIV, "", "◾ *채널분류별*", ""]
    ...
    #   채널명 하이퍼링크 = 그날 채널별 최고 증분 소재. 라벨로 안내.
    lines.append("`CPV는 누적 기준 / 하이퍼링크 금일 채널별 BEST`")
    lines.append("")
