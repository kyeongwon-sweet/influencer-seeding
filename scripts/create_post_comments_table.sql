...
create table if not exists public.post_comments (
  ...
  handled_at     timestamptz,         -- 슬랙 버튼으로 처리한 시각
  handled_by     text,                -- 처리한 슬랙 사용자
  handled_action text,                -- done | ignored
  created_at     timestamptz not null default now(),
  unique (post_id, comment_id)
);

-- 이미 post_comments가 있으면 처리상태 컬럼만 추가:
alter table public.post_comments add column if not exists handled_at timestamptz;
alter table public.post_comments add column if not exists handled_by text;
alter table public.post_comments add column if not exists handled_action text;
