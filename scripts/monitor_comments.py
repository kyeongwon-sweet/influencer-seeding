...
#   3) post_comments에 없는 신규 댓글만 분류 = '라라스윗/쫀득바(제품·브랜드) 부정'만 탐지.
#      문맥 판단이 필요(욕설도 애정표현이면 normal, 저당이라 '별로 안달다'=칭찬) → Claude 권장.
#      ANTHROPIC_API_KEY 없으면 명백한 제품불만/CS 문구만 잡는 고정밀 폴백(놓침 많음).
...
# ⚠️ 부정 판단 기준 = '라라스윗/쫀득바(제품·브랜드)에 대한 실제 부정'. 욕설·강조어·모호어는
#    문맥 없이는 판별 불가(예: "미쳤네"=감탄, "처먹었는데"=추억, 저당브랜드라 "별로 안달다"=칭찬).
#    → 문맥 판단은 Claude가 담당. 아래 폴백(LLM 키 없을 때만)은 '명백한 제품 불만/CS 클레임' 문구만
#    고정밀로 잡고, 애매한 것(광고·바이럴·별로·욕설 단독)은 일부러 넣지 않는다(오탐 방지).
NEG_KEYWORDS = (
    # 맛·품질 혹평(명백)
    "맛없", "노맛", "존맛없", "최악", "비추", "사먹지 마", "사지 마세요", "돈 아깝", "돈아깝",
    "과대광고", "뒷광고", "바가지", "창렬", "가성비 최악",
    # CS·위생 클레임(대응 필요)
    "환불", "이물", "머리카락", "벌레", "곰팡이", "곰팡", "상했", "상한 거", "썩은", "불량",
    "배탈", "설사", "토할", "토했", "녹아서 왔", "터져서 왔", "누락됐", "안 왔어요", "안왔어요",
    "고객센터", "문의했는데", "답이 없",
)


def _classify_fallback(comments: list) -> list:
    """LLM 키 없을 때만 쓰는 고정밀 폴백. 명백한 제품 불만/CS 문구만 negative, 나머지 normal.
    ⚠️ 문맥이 필요한 판단(욕설·광고조롱·반어·'별로')은 놓친다 → ANTHROPIC_API_KEY 설정 필수."""
    out = []
    for c in comments:
        t = (c.get("text") or "").replace(" ", "")
        hit = next((k for k in NEG_KEYWORDS if k.replace(" ", "") in t), None)
        label = "issue" if hit in ("환불", "이물", "머리카락", "벌레", "곰팡이", "곰팡", "배탈",
                                   "설사", "불량", "고객센터", "문의했는데", "답이 없") else \
                ("negative" if hit else "normal")
        out.append({"label": label, "reason": f"제품 불만 키워드 '{hit}'" if hit else None})
    return out
...
def _classify_claude(comments: list) -> list | None:
    ...
    for chunk in _chunks(comments, 25):
        ...
        prompt = (
            "당신은 '라라스윗'(저당·무설탕 아이스크림/디저트 브랜드, 대표 제품 '쫀득바' 등) 마케팅팀의 SNS 댓글 검토 담당입니다.\n"
            "협찬/광고 게시물 댓글 중, 팀이 실제로 '관리(대응)'해야 할 부정 댓글만 골라내세요.\n"
            "핵심 원칙: **라라스윗 브랜드 또는 그 제품(쫀득바 등)에 대한 실제 부정**일 때만 negative/issue. 애매하면 normal.\n\n"
            "[negative — 라라스윗/제품에 대한 부정]\n"
            "  · 맛·식감·품질·양·가격 혹평, 구매 만류('사지 마'), 성분·효능 의혹\n"
            "  · 라라스윗을 향한 광고/협찬 조롱·상술 비판('이거 다 광고네', '뒷광고', '끼워팔기', '낚시')\n"
            "  · 라라스윗/제품·관계자를 향한 욕설·조롱·인신공격(우회표기 포함)\n"
            "[issue — 대응 필요한 제품 CS 클레임]\n"
            "  · 이물·변질·배탈 등 안전, 배송·파손·누락, 환불 요구, 문의 무응답\n"
            "[normal — 관리 대상 아님(대부분 여기)]\n"
            "  · 긍정·중립·감탄. **욕설/강조어라도 부정 의미가 아니면 normal**('미쳤다 맛있어', '존맛', '개맛있음', '미쳤네').\n"
            "  · 저당 브랜드 특성상 '별로 안 달아요/안 달다/덜 달다'는 **칭찬 → normal**.\n"
            "  · 제품과 무관한 잡담·추억·유머·태그·이모지, 인플루언서 개인 얘기(제품 부정 아님).\n\n"
            "[관리 대상 아님 예시 → 반드시 normal]\n"
            "  · '미쳤네...😲' (감탄)\n"
            "  · '카스테라 종이는 진짜 어릴때 먹는건줄 알고 맨날 맛있다고 처먹었는데' (추억·긍정, 제품 비판 아님)\n"
            "  · '존나 웃김' / '이거 존맛' (욕설이지만 부정 아님)\n\n"
            f"댓글 목록:\n{numbered}\n\n"
