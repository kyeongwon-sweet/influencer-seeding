...
여러 Claude 세션 + Codex + Apps Script 자동화가 **같은 DB·구글시트·main 브랜치**를 동시에 쓴다("단일 소유자" 전제는 거짓). 아래는 이번 다중세션 사고들의 재발방지 합의. **어길 시 비가역 데이터 손실.**

1. **데이터 절대규칙(무결성).** 실측 없으면 값 지어내기 금지 · 수집불가/미측정이면 **비워둠**(마지막값 복사·타 게시물 값·추정치 저장 금지) · 이상치 자동보정 금지(**감지 알림만**, 사람이 정정) · ⚠️ **빈 값(측정 없음)을 0으로 읽지 말 것** · **`posted_at`(게시일) 절대 자동수정 금지**(정합성 문제는 백필삭제+가드로).
2. **삭제/정정 전 트리아지 3원칙.** (a) **오귀속 금지** — 시트 편집기록의 계정명(예 "황경원")은 사람이 아니라 `exportStats` 등 자동 스크립트가 그 계정으로 쓴 것일 수 있음. `created_at`+transcript로 실제 출처 확인. (b) **전 컬럼 검증** — 한 지표만 보고 판단 금지(play_count만 보다 배너 `reach_count` 놓칠 뻔). (c) **검증 전 삭제 금지** — 사람이 "지워"라 해도 실측으로 진위 확인 전엔 보류. 재수집 불가 지표(reach=IG인사이트, 틱톡 POST_SENSITIVE)는 팀 확인. 값이 진짜면 삭제=실데이터 손실.
3. **DB 대량수정 금지.** 쓰기 전 **소비 코드부터 읽고 표시값 재현**(화면이 이미 보정할 수 있음) → **최소 범위**(전량 UPDATE/DELETE 금지, WHERE로 대상만) → **백업 먼저** → 대량/비가역은 **사용자 사전확인**. (NULL 채움이 의도 30행→2,497행 오삽입된 사고 있음. 되돌릴 백업 없으면 실행 금지.)
4. **시트 쓰기는 행 위치 아닌 URL(linkKey) 기준.** 라이브 Apps Script는 저장이 원자적이 아니라, 스크립트 실행 중 다른 세션이 행 삽입/편집하면 **행 위치가 밀려 누적/날짜값이 옆 행에 기입**됨(off-by-one write race). 값은 항상 URL/키로 매칭해 쓰고, 쓰기 직전 위치 재확인.
5. **프로덕션 배포는 단일 경로·동시 배포 금지.** 배포(`vercel --prod`, 수동 CLI)는 **Codex 소유**. 배포 전 진행 중인 배포 없는지 확인, main HEAD 기준, 한 번에 한 세션만. (내 worktree CLI 배포는 UNKNOWN으로 실패한 이력 — 라이브 반영 여부는 `-mu` 도메인 직접 확인.)
6. **작업 전 `AI_SHARED_STATUS.md` 필독, 후 갱신.** 내가 한 것/못 한 것·미검증을 **정직하게** 기록. "내 탓 아님/다른 세션 탓" 금지 — 멀티세션 결과물도 공동 책임으로 소유·정합화.
7. **시크릿 비출력.** 토큰/키(SERVICE_ROLE·CRON_SECRET·SLACK·Signing) 채팅 출력·git추적 파일 기재 금지(repo PUBLIC). 위치로만 로드.
